...
const AcademicFacultyDeptails = () => {
  ...
  const { data, isLoading } = useAcademicFacultysQuery({ ...query });
  ...
  const DeletedHanler = async (id: string) => {
    ...
    try {
      ...
      message.success("Department Deleted SuccessFull");
    } catch (err: any) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: function (data: any) {
        return (
          <>
            <Link href={`/admin/academic/faculty/edite/${data?.id}`}>
              <Button
...
export default AcademicFacultyDeptails;
